    ...
    <Card>
      <CardContent className="p-6 space-y-6">
        ...
        {(() => {
          ...
          const upcomingPerks = nextPerks(level, 3);
          return (
            <div>
              <div className="flex items-center gap-2 mb-3">
                <Sparkles className="w-3.5 h-3.5 text-muted-foreground" />
                <h3 className="text-sm font-display font-semibold">Perks</h3>
                <span className="text-[10px] text-muted-foreground ml-auto">
                  {earned.length} unlocked
                </span>
              </div>
              <div className="space-y-1.5">
                {earned.slice(-3).reverse().map((p) => {
                  const Icon = p.icon;
                  return (
                    <div
                      key={p.key}
                      className="flex items-center gap-2.5 py-2 px-3 rounded-lg border border-primary/30 bg-primary/5"
                    >
                ...
                {upcomingPerks.map((p) => {
                  ...
                  return (
                    <div
                      key={p.key}
                      className="flex items-center gap-2.5 py-2 px-3 rounded-lg bg-muted/30 opacity-80"
                    >
                ...
                {earned.length === 0 && upcomingPerks.length === 0 && (
                  <div className="text-xs text-muted-foreground py-4 text-center">
                    All perks unlocked — you're a Scholar!
                  </div>
                )}
              </div>
            </div>
