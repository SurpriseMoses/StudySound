...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "translation_health") {
      ...
      const { data: rows, error } = await admin.rpc("admin_translation_health", {
        _document_id: document_id ?? null,
      ...
      return json({
        ...
        documents: (rows ?? []).map((r: { document_id: string; total: number; leaked: number; stale_version: number; missing_hash: number }) => ({
          document_id: r.document_id,
          total: Number(r.total),
          leaked: Number(r.leaked),
          stale_version: Number(r.stale_version),
          missing_hash: Number(r.missing_hash),
        })),
      });
    }
    ...
    if (action === "reprocess_dirty_translations") {
      ...
      const { data, error } = await admin
        ...
        .delete({ count: "exact" })
        .eq("document_id", document_id)
        .or(`english_leak_detected.eq.true,translation_version.lt.${CURRENT_VERSION},source_text_hash.is.null`)
        .select("id");
      if (error) throw error;
