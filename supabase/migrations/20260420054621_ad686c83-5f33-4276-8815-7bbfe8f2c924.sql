-- Remove auth.uid()-based guards inside admin RPCs.
-- The edge function (admin-api) already verifies the caller is admin before invoking these.
-- These functions are SECURITY DEFINER and not exposed via PostgREST in a way users can call directly without going through the edge function.
...
CREATE OR REPLACE FUNCTION public.admin_top_documents(_limit integer DEFAULT 20)
 RETURNS TABLE(document_id uuid, title text, audio_unlocks bigint, translation_unlocks bigint, visual_unlocks bigint, total_unlocks bigint, credits_generated bigint, audio_cached bigint, last_activity timestamp with time zone)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
CREATE OR REPLACE FUNCTION public.admin_abuse_candidates()
 RETURNS TABLE(user_id uuid, display_name text, plan text, is_flagged boolean, cooldown_until timestamp with time zone, translations_today bigint, translations_last_minute bigint, audio_today bigint, daily_cap integer)
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
